function handles = barweb_lu(barvalues, L, U, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend)
...
if size(barvalues,1) ~= size(L,1) || size(barvalues,2) ~= size(L,2)
	...
	if size(barvalues,1) == 1
		...
		L = [L; zeros(1,size(barvalues,2))];
		U = [U; zeros(1,size(barvalues,2))];        
		change_axis = 1;
	end
	...
	for i = 1:numbars
		...
		% handles.errors(i) = errorbar(x, barvalues(:,i), errors(:,i), 'k', 'linestyle', 'none');
                he = errorbar(x, barvalues(:,i), L(:,i), U(:,i),'k.');
                set(he,'Marker','none');
	end
	set(gcf, 'color', 'white')
	...
	% set(gca, 'xticklabel', groupnames, 'box', 'off', 'ticklength', [0.1 0.1], 'fontsize', 12, 'xtick',1:numgroups);
    set(gca, 'xticklabel', groupnames, 'box', 'off', 'fontsize', 12, 'xtick',1:numgroups);
