function [slope,intercept,STAT,CIR]=ig_myregr(x,y,verbose,check_outliers)
% modified by IK:
% Disable outlier check: check outliers: 0 - do not check, 1 - check interactively (as in original code), 2 - check and remove without asking
% Return 95% CI of regression: CIR.xaxis and CIT.yaxis
...
%check the presence of outliers % IK
outl=find(ismember(sign(Rint),[-1 1],'rows')==0);
if ~isempty(outl) && check_outliers 
	if check_outliers==1,
