function [handles,x_axis,y,e] = barweb(barvalues, errors, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend)
% modified by Igor Kagan, 2013
%
...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
	...
	for i = 1:numbars
		...
		% handles.errors(i) = errorbar(x, barvalues(:,i), errors(:,i), 'k', 'linestyle', 'none');
                he = errorbar(x, barvalues(:,i), errors(:,i),'k.');
                set(he,'Marker','none');
		x_axis = [x_axis x];
	end
	
	if change_axis,
		x_axis = x_axis(1:2:end);
	end
